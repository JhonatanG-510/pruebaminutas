...
/* ── REDES DE SEGURIDAD para flujo móvil ─────────────────────────── */
(function () {
  var fallbackTimer = null;

  function ensureEmergencyClose() {
    if (document.getElementById("mlc-emergency-close")) return;
    var btn = document.createElement("button");
    btn.id = "mlc-emergency-close";
    btn.type = "button";
  ...
  function showPreviewFallback() {
    ...
    if (!cont) return;
    if (document.getElementById("live-preview-doc")) return;

    cont.innerHTML =
      '<div class="mlc-preview-fallback">' +
        '<h4>La previsualizaci\u00f3n est\u00e1 tardando demasiado</h4>' +
        '<p>Puedes seguir con el formulario tradicional. Funciona igual y se descarga el mismo documento.</p>' +
        '<button type="button" id="mlc-fallback-btn">Usar formulario cl\u00e1sico</button>' +
      '</div>';

    var btn = document.getElementById("mlc-fallback-btn");
    if (btn) {
      btn.onclick = function () {
        document.body.classList.remove("mlc-mobile-flow", "mlc-mobile-paypanel");
  ...
  function startFallbackTimer() {
    ...
    fallbackTimer = setTimeout(showPreviewFallback, 7000);
  }

  function initSafety() {
    var ov = document.getElementById("modal-overlay");
    if (!ov) { setTimeout(initSafety, 200); return; }
    ...
    new MutationObserver(function () {
      if (ov.classList.contains("open")) {
        ...
        clearTimeout(fallbackTimer);
        var btn = document.getElementById("mlc-emergency-close");
