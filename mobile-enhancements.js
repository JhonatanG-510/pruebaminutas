...
/* ══════════════════════════════════════════════════════════════════
   PARCHE — Previsualización rápida (PizZip) + Web Worker para mammoth
            + Barra de emergencia siempre visible en el flujo móvil
══════════════════════════════════════════════════════════════════ */
(function () {

  /* ── 1. Sustituir inicializarLivePreview por una versión RÁPIDA
         que usa PizZip + extracción de texto, sin mammoth.
         (mammoth se queda solo para las otras previsualizaciones). ── */
  ...
  function _mlcBuildHtmlFromDocxXml(xml) {
    ...
    while ((m = parrafoRe.exec(limpio)) !== null) {
      ...
      if (texto.trim()) {
        html.push("<p>" + _mlcEscHtml(texto) + "</p>");
      } else {
        html.push("<p>&nbsp;</p>");
      }
    }
  ...
  // Esperar a que app.js termine de definir sus funciones antes de sobrescribir
  function instalarLivePreviewRapido() {
    if (typeof window.PizZip === "undefined") {
      // PizZip aún no cargó, reintentar
      return setTimeout(instalarLivePreviewRapido, 200);
    }
    ...
    window.inicializarLivePreview = async function () {
      var cont = document.getElementById("live-preview-content");
      if (!cont) return;

      var blob = (typeof docxBlob !== "undefined") ? docxBlob : null;
      if (!blob) {
        cont.innerHTML = '<div class="live-preview-empty">No fue posible cargar la previsualización del documento.</div>';
        if (typeof aplicarClaseLivePreviewSegunPaso === "function") aplicarClaseLivePreviewSegunPaso();
        return;
      }
      ...
      // Ceder el hilo para que se pinte el spinner
      ...
      try {
        ...
        if (!html) {
          cont.innerHTML = '<div class="live-preview-empty">El documento no contiene contenido visible.</div>';
          if (typeof aplicarClaseLivePreviewSegunPaso === "function") aplicarClaseLivePreviewSegunPaso();
          return;
        }

        cont.innerHTML = '<div class="word-page" id="live-preview-doc">' + html + '</div>';
        var doc = cont.querySelector("#live-preview-doc");
        if (!doc) return;

        if (typeof _lpEnvolverClausulasEnDom === "function") _lpEnvolverClausulasEnDom(doc);
        if (typeof _lpEnvolverPlaceholdersEnDom === "function" &&
            typeof _lpRecolectarPlaceholders === "function") {
          _lpEnvolverPlaceholdersEnDom(doc, _lpRecolectarPlaceholders());
        }

        livePreviewReady = true;
        if (typeof wireLivePreviewInputs === "function") wireLivePreviewInputs();
        if (typeof protegerLivePreviewAntiCopia === "function") protegerLivePreviewAntiCopia();
        if (typeof aplicarClaseLivePreviewSegunPaso === "function") aplicarClaseLivePreviewSegunPaso();
        if (typeof actualizarLivePreview === "function") actualizarLivePreview();
      } catch (e) {
        console.error("[livePreview-rapido]", e);
        cont.innerHTML = '<div class="live-preview-empty">No se pudo generar la previsualización: ' + _mlcEscHtml(e.message || String(e)) + '</div>';
        if (typeof aplicarClaseLivePreviewSegunPaso === "function") aplicarClaseLivePreviewSegunPaso();
      }
    };
    console.log("[MLC] inicializarLivePreview reemplazado por versión rápida (PizZip)");
  }

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", instalarLivePreviewRapido);
  } else {
    instalarLivePreviewRapido();
  }

  /* ── 2. Web Worker para mammoth (las OTRAS 5 previsualizaciones) ── */
  if (typeof Worker !== "undefined") {
    var workerCode =
      "self.importScripts('https://cdn.jsdelivr.net/npm/mammoth@1.6.0/mammoth.browser.min.js');" +
      "self.onmessage = function(e){" +
      "  try {" +
      "    self.mammoth.convertToHtml({arrayBuffer: e.data}).then(function(r){" +
      "      self.postMessage({ok:true, html:r.value, messages:r.messages||[]});" +
      "    }).catch(function(err){" +
      "      self.postMessage({ok:false, error: String(err && err.message || err)});" +
      "    });" +
      "  } catch(syncErr) {" +
      "    self.postMessage({ok:false, error: 'sync:' + (syncErr.message || syncErr)});" +
      "  }" +
      "};";
    ...
    function waitForMammoth(cb, tries) {
      tries = tries || 0;
      if (typeof window.mammoth !== "undefined") return cb();
      if (tries > 40) return;
      setTimeout(function () { waitForMammoth(cb, tries + 1); }, 300);
    }

    if (workerUrl) {
      waitForMammoth(function () {
        var orig = window.mammoth.convertToHtml;
        window.mammoth.convertToHtml = function (input) {
          return new Promise(function (resolve, reject) {
            var buf = input && input.arrayBuffer;
            if (!buf) { reject(new Error("Sin arrayBuffer")); return; }

            var w;
            try { w = new Worker(workerUrl); }
            catch (werr) { reject(werr); return; }
            ...
            w.onmessage = function (e) {
              ...
              try { w.terminate(); } catch (_) {}
              if (e.data && e.data.ok) {
                resolve({ value: e.data.html || "", messages: e.data.messages || [] });
              } else {
                reject(new Error((e.data && e.data.error) || "Error en worker"));
              }
            };
            w.onerror = function (e) {
              ...
              clearTimeout(timer);
              try { w.terminate(); } catch (_) {}
              reject(new Error(e.message || "Error en worker"));
            };

            try {
              var copy = buf.slice(0);
              w.postMessage(copy, [copy]);
            } catch (postErr) {
              done = true;
              clearTimeout(timer);
              try { w.terminate(); } catch (_) {}
  ...
  /* ── 3. Barra de emergencia siempre visible (cerrar + clásico) ── */
  ...
  function ensureEmergencyBar() {
    ...
    btnClose.id = "mlc-btn-emergency-close";
    btnClose.type = "button";
    btnClose.textContent = "\u2715  Cerrar";
    bindBoth(btnClose, cerrarModalForzado);

    var btnFallback = document.createElement("button");
    btnFallback.id = "mlc-btn-emergency-fallback";
    btnFallback.type = "button";
    btnFallback.textContent = "Usar formulario cl\u00e1sico";
    bindBoth(btnFallback, usarFormularioClasico);

    bar.appendChild(btnClose);
    bar.appendChild(btnFallback);
  ...
  function watchModal() {
    ...
    function check() {
      if (ov.classList.contains("open") && document.body.classList.contains("mlc-mobile-flow")) {
        ensureEmergencyBar();
      } else {
        removeEmergencyBar();
      }
