...
/* ── BARRA DE EMERGENCIA SIEMPRE VISIBLE en flujo móvil ─────────── */
(function () {
...
(function () {
  function ensureDebugPanel() {
    var d = document.getElementById("mlc-debug");
    if (d) return d;
    ...
    close.style.cssText = "position:absolute;bottom:6px;right:10px;color:#aaa;font-size:10px;cursor:pointer;text-decoration:underline;";
  ...
  function log(msg) {
    var d = ensureDebugPanel();
    var t = new Date().toLocaleTimeString();
    var line = document.createElement("div");
    line.textContent = "[" + t + "] " + msg;
    d.insertBefore(line, d.firstChild);
    try { console.log("[MLC-DBG]", msg); } catch (_) {}
  }

  log("Diagnóstico iniciado. Esperando mammoth...");

  // Verificar mammoth al cargar (con reintentos)
  var attempts = 0;
  var checkInterval = setInterval(function () {
    attempts++;
    if (typeof window.mammoth !== "undefined") {
      clearInterval(checkInterval);
      log("OK: mammoth cargado tras " + attempts + " intento(s)");
      ...
      window.mammoth.convertToHtml = function (input) {
        ...
        var size = "?";
        try {
          if (input && input.arrayBuffer) size = input.arrayBuffer.byteLength + " bytes";
        } catch (_) {}
        log(">> mammoth.convertToHtml llamado, input=" + size);

        var watcher = setInterval(function () {
          log("   ...mammoth lleva " + Math.round((Date.now() - t0)/1000) + "s trabajando");
        }, 2000);

        try {
          var p = orig.call(window.mammoth, input);
          var timeout = new Promise(function (_, reject) {
            setTimeout(function () { reject(new Error("TIMEOUT 10s — mammoth no respondió")); }, 10000);
          });
          return Promise.race([p, timeout]).then(function (res) {
            clearInterval(watcher);
            var ms = Date.now() - t0;
            var html = (res && res.value) ? res.value : "";
            log("<< OK mammoth: " + ms + "ms, HTML=" + html.length + " chars");
            if (res && res.messages && res.messages.length) {
              log("   mammoth msgs: " + res.messages.length);
              res.messages.slice(0, 3).forEach(function (m) {
                log("     - " + m.type + ": " + (m.message || "").slice(0, 80));
              });
            }
            return res;
          }, function (err) {
            clearInterval(watcher);
            log("XX ERROR mammoth: " + (err && err.message ? err.message : String(err)));
            throw err;
          });
        } catch (syncErr) {
          clearInterval(watcher);
          log("XX EXCEPCIÓN sincrónica en mammoth: " + syncErr.message);
          throw syncErr;
        }
      };
    } else if (attempts > 30) {
      clearInterval(checkInterval);
      log("XX ERROR: mammoth NUNCA cargó tras 15s. CDN bloqueado o red caída.");
    }
  }, 500);

  // Monitorear cambios en live-preview-content para ver el flujo
  function watchPreview() {
    var cont = document.getElementById("live-preview-content");
    if (!cont) { setTimeout(watchPreview, 300); return; }
    var lastSig = "";
    new MutationObserver(function () {
      var sig = (cont.textContent || "").slice(0, 60).replace(/\s+/g, " ").trim();
      if (sig !== lastSig) {
        lastSig = sig;
        log("preview-content: \"" + sig + "...\"");
      }
    }).observe(cont, { childList: true, subtree: true, characterData: true });
    log("Vigilando live-preview-content");
  }
  watchPreview();

  // Interceptar fetch para ver si la URL del docx falla
  var origFetch = window.fetch;
  if (origFetch) {
    window.fetch = function (url, opts) {
      var u = String(url);
      if (u.indexOf(".docx") !== -1 || u.indexOf("firebasestorage") !== -1) {
        log("fetch DOCX -> " + u.slice(-80));
        var t0 = Date.now();
        return origFetch.apply(this, arguments).then(function (r) {
          log("fetch DOCX <- status " + r.status + " en " + (Date.now()-t0) + "ms");
          return r;
        }, function (e) {
          log("fetch DOCX XX ERROR: " + e.message);
          throw e;
        });
      }
      return origFetch.apply(this, arguments);
    };
    log("Interceptor de fetch activo");
  }
})();
