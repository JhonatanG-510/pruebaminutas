...
/* ══════════════════════════════════════════════════════════════════
   PARCHE — Reemplazar mammoth.convertToHtml por una versión rápida
            basada en PizZip (sin bloqueo del navegador).
            + Barra de emergencia siempre visible.
            + Panel de diagnóstico mínimo (esquina inferior izquierda).
══════════════════════════════════════════════════════════════════ */
(function () {
  ...
  /* ── Mini panel de estado (inferior izquierda) ── */
  var statusEl = null;
  function setStatus(txt, color) {
    if (!statusEl) {
      ...
      statusEl.style.cssText = [
        "position:fixed","bottom:80px","left:8px","z-index:99996",
  ...
  function intentarReemplazarMammoth(tries) {
    ...
    if (typeof window.PizZip === "undefined" || typeof window.mammoth === "undefined") {
      if (tries > 50) {
        setStatus("ERROR: PizZip o mammoth no cargaron", "#ff6");
        return;
      }
    ...
    window.mammoth.convertToHtml = function (input) {
      return new Promise(function (resolve, reject) {
        ...
        var t0 = Date.now();
        setStatus("Convirtiendo .docx con PizZip...", "#9f9");

        // Ceder el hilo para que el spinner se pinte primero
        setTimeout(function () {
          try {
            var zip = new PizZip(buf);
            if (!zip.files["word/document.xml"]) {
              setStatus("ERROR: no hay word/document.xml en el zip", "#f99");
            ...
            var ms = Date.now() - t0;
            setStatus("OK convertido en " + ms + "ms (" + html.length + " chars)", "#9f9");
            setTimeout(function () { setStatus("Listo", "#9f9"); }, 3000);
            resolve({ value: html, messages: [] });
          } catch (e) {
            setStatus("ERROR PizZip: " + (e.message || e), "#f99");
            reject(e);
          }
        }, 0);
      });
    };
    setStatus("mammoth reemplazado por PizZip OK", "#9f9");
  }
  ...
  /* ── 2. Barra de emergencia siempre visible ── */
