...
/* ══════════════════════════════════════════════════════════════════
   PARCHE — Diagnóstico completo + reemplazo PizZip
══════════════════════════════════════════════════════════════════ */
(function () {

  /* ── Panel de estado MUY visible (parte superior) ── */
  var statusEl = null;
  var statusLines = [];
  function setStatus(txt) {
    statusLines.push(new Date().toISOString().substr(14, 9) + " " + txt);
    if (statusLines.length > 8) statusLines.shift();
    if (!statusEl) {
      statusEl = document.createElement("div");
      statusEl.id = "mlc-status";
      statusEl.style.cssText = [
        "position:fixed","top:0","left:0","right:0",
        "z-index:2147483647",
        "background:rgba(0,0,0,0.92)","color:#0f0",
        "font:11px/1.35 ui-monospace,Menlo,monospace",
        "padding:6px 10px","max-height:40vh","overflow:auto",
        "white-space:pre-wrap","pointer-events:auto",
        "border-bottom:2px solid #0f0"
      ].join(";");
      if (document.body) document.body.appendChild(statusEl);
      else document.addEventListener("DOMContentLoaded", function(){ document.body.appendChild(statusEl); });
    }
    statusEl.textContent = "[MLC] DIAGNÓSTICO\n" + statusLines.join("\n");
    try { console.log("[MLC]", txt); } catch (_) {}
  }

  /* ── Heartbeat: prueba que el hilo principal no está congelado ── */
  var hbCount = 0;
  setInterval(function () { hbCount++; }, 500);
  function hb() { return "(hb=" + hbCount + ")"; }

  /* ── Helpers de extracción XML ── */
  function _mlcEscHtml(s) {
    return String(s).replace(/&/g, "&amp;").replace(/</g, "&lt;")
                    .replace(/>/g, "&gt;").replace(/"/g, "&quot;");
  }
  function _mlcBuildHtmlFromDocxXml(xml) {
    var limpio = xml
    ...
    var html = [];
    var parrafoRe = /<w:p\b[\s\S]*?<\/w:p>/g, m;
    while ((m = parrafoRe.exec(limpio)) !== null) {
      var pXml = m[0];
      var matches = pXml.match(/<w:t[^>]*>[^<]*<\/w:t>/g) || [];
      var texto = matches.map(function (frag) {
        return frag.replace(/^<w:t[^>]*>/, "").replace(/<\/w:t>$/, "");
      }).join("");
      if (texto.trim()) html.push("<p>" + _mlcEscHtml(texto) + "</p>");
      else html.push("<p>&nbsp;</p>");
    }
    return html.join("");
  }

  /* ── 1. Reemplazar mammoth.convertToHtml ── */
  function intentarReemplazarMammoth(tries) {
    tries = tries || 0;
    if (typeof window.PizZip === "undefined" || typeof window.mammoth === "undefined") {
      if (tries > 50) { setStatus("ERROR: PizZip o mammoth NO cargaron " + hb()); return; }
      setTimeout(function () { intentarReemplazarMammoth(tries + 1); }, 200);
      return;
    }
    setStatus("PizZip+mammoth disponibles. Parcheando... " + hb());

    window.mammoth.convertToHtml = function (input) {
      return new Promise(function (resolve, reject) {
        var buf = input && input.arrayBuffer;
        if (!buf) { setStatus("convertToHtml: SIN arrayBuffer"); reject(new Error("Sin arrayBuffer")); return; }
        setStatus("convertToHtml llamado, buf=" + buf.byteLength + "B " + hb());
        setTimeout(function () {
          try {
            setStatus("PizZip parseando... " + hb());
            var t0 = Date.now();
            var zip = new PizZip(buf);
            setStatus("PizZip OK en " + (Date.now()-t0) + "ms " + hb());
            if (!zip.files["word/document.xml"]) { setStatus("ERROR: no hay document.xml"); resolve({ value: "", messages: [] }); return; }
            setStatus("Leyendo XML... " + hb());
            var xml = zip.files["word/document.xml"].asText();
            setStatus("XML leído (" + xml.length + " chars) " + hb());
            var html = _mlcBuildHtmlFromDocxXml(xml);
            setStatus("HTML construido (" + html.length + " chars) " + hb());
            resolve({ value: html, messages: [] });
          } catch (e) {
            setStatus("ERROR PizZip: " + (e.message || e));
            reject(e);
          }
        }, 0);
      });
    };
    setStatus("mammoth REEMPLAZADO " + hb());

    /* ── 2. Envolver inicializarLivePreview también, para diagnóstico ── */
    if (typeof window.inicializarLivePreview === "function") {
      var orig = window.inicializarLivePreview;
      window.inicializarLivePreview = async function () {
        setStatus("inicializarLivePreview INICIADO " + hb());
        try {
          var r = await orig.apply(this, arguments);
          setStatus("inicializarLivePreview TERMINADO " + hb());
          return r;
        } catch (e) {
          setStatus("inicializarLivePreview ERROR: " + (e.message||e));
          throw e;
        }
      };
      setStatus("inicializarLivePreview envuelto " + hb());
    } else {
      setStatus("AVISO: inicializarLivePreview NO está en window");
    }
  }

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", function () { intentarReemplazarMammoth(0); });
  } else {
    intentarReemplazarMammoth(0);
  }

  /* ── Botón para ocultar el panel de diagnóstico ── */
  document.addEventListener("DOMContentLoaded", function () {
    var btn = document.createElement("button");
    btn.textContent = "Ocultar diagnóstico";
    btn.style.cssText = "position:fixed;top:6px;right:6px;z-index:2147483647;background:#fff;color:#000;border:0;padding:6px 10px;border-radius:6px;font:11px sans-serif;";
    btn.addEventListener("click", function () {
      if (statusEl) statusEl.style.display = (statusEl.style.display === "none" ? "" : "none");
    });
    document.body.appendChild(btn);
  });

  /* ── 3. Barra de emergencia siempre visible ── */
  function cerrarModalForzado() {
    try {
      ...
      document.body.classList.remove("mlc-mobile-flow", "mlc-mobile-paypanel");
      var bar = document.getElementById("mlc-emergency-bar");
      if (bar && bar.parentNode) bar.parentNode.removeChild(bar);
    } catch (e) {}
  }
  function usarFormularioClasico() {
    try {
      document.body.classList.remove("mlc-mobile-flow", "mlc-mobile-paypanel");
      ["steps-bar-container","step-1","step-2","step-clausulas","step-3","live-preview-toggle-mobile"]
        .forEach(function (id) { var el = document.getElementById(id); if (el) el.style.removeProperty("display"); });
      var bar = document.getElementById("mlc-emergency-bar");
      if (bar && bar.parentNode) bar.parentNode.removeChild(bar);
    } catch (e) {}
  }
  function bindBoth(el, fn) {
    var fired = false;
    function go(e) {
      if (fired) return;
      fired = true; setTimeout(function () { fired = false; }, 600);
  ...
  function ensureEmergencyBar() {
    if (document.getElementById("mlc-emergency-bar")) return;
    var bar = document.createElement("div");
    bar.id = "mlc-emergency-bar";
    var btnClose = document.createElement("button");
    btnClose.id = "mlc-btn-emergency-close"; btnClose.type = "button";
    btnClose.textContent = "\u2715  Cerrar"; bindBoth(btnClose, cerrarModalForzado);
    var btnFallback = document.createElement("button");
    btnFallback.id = "mlc-btn-emergency-fallback"; btnFallback.type = "button";
    btnFallback.textContent = "Usar formulario cl\u00e1sico"; bindBoth(btnFallback, usarFormularioClasico);
    bar.appendChild(btnClose); bar.appendChild(btnFallback); document.body.appendChild(bar);
  }
  function removeEmergencyBar() {
    var bar = document.getElementById("mlc-emergency-bar");
    if (bar && bar.parentNode) bar.parentNode.removeChild(bar);
  }
  function watchModal() {
    var ov = document.getElementById("modal-overlay");
    if (!ov) { setTimeout(watchModal, 200); return; }
    function check() {
      if (ov.classList.contains("open") && document.body.classList.contains("mlc-mobile-flow")) ensureEmergencyBar();
      else removeEmergencyBar();
    }
    new MutationObserver(check).observe(ov, { attributes: true, attributeFilter: ["class"] });
