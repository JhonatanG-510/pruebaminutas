...
/* Caché de contextos por campo para no re-parsear el docx cada vez */
let contextosCamposCache = {};
...
document.getElementById("form-login").addEventListener("submit", async e => {
  ...
  try {
    ...
    if (pendingMinutaId) {
      ...
      const savedCampos = { ...pendingCamposSnapshot };
      pendingMinutaId = null; pendingCamposSnapshot = null;
      toast("¡Bienvenido! Continúa donde lo dejaste.", "ok");
      await abrirMinuta(savedId);
      camposLlenados = savedCampos;
      const pagoIdx = getFlowPanels().indexOf("4");
      if (pagoIdx !== -1) renderStep(pagoIdx + 1);
    } else {
      toast("¡Bienvenido!", "ok");
      showSection("inicio");
    }
...
function renderCamposPage() {
  ...
  slice.forEach(({ nombre: campo, tipo }) => {
    ...
    const ctxId  = "ctx_" + campo.replace(/[^a-zA-Z0-9]/g, "_");
    const campoJs = campo.replace(/\\/g, "\\\\").replace(/'/g, "\\'");
    const btnCtx = `<button type="button" class="btn-campo-contexto" onclick="mostrarContextoCampo('${campoJs}')" title="Ver cómo aparece este campo en el documento">👁 Ver contexto</button>`;
    if (tipo === "largo") {
      div.innerHTML = `
        ...
        <div class="campo-contexto-box" id="${ctxId}" style="display:none;"></div>
...
/* Extrae ~10 palabras antes y ~10 después del marcador dentro de su párrafo.
   Devuelve { antes:"...", marcador:"TEXTO EXACTO", despues:"..." } o null. */
async function extraerContextoCampo(campo) {
  if (contextosCamposCache[campo] !== undefined) return contextosCamposCache[campo];
  if (!docxBlob) { contextosCamposCache[campo] = null; return null; }
  try {
    const buf  = await docxBlob.arrayBuffer();
    const zip  = new PizZip(buf);
    if (!zip.files["word/document.xml"]) { contextosCamposCache[campo] = null; return null; }
    ...
    while ((m = parrafoRe.exec(limpio)) !== null) {
      const t = extractTextFromDocxXml(m[0]).trim();
      if (t) textos.push(t);
    }
    const campoUpper = campo.toUpperCase();
    let parrafo = null;
    for (const t of textos) {
      if (t.toUpperCase().includes(campoUpper)) { parrafo = t; break; }
    }
    if (!parrafo) { contextosCamposCache[campo] = null; return null; }

    // Localizar la posición exacta del marcador (case-insensitive)
    const pos = parrafo.toUpperCase().indexOf(campoUpper);
    const marcador = parrafo.slice(pos, pos + campo.length);
    const txtAntes  = parrafo.slice(0, pos);
    const txtDespues = parrafo.slice(pos + campo.length);

    // Tomar las últimas 10 palabras antes y las primeras 10 después
    const PALABRAS = 10;
    const palabrasAntes  = txtAntes.trim().split(/\s+/).filter(Boolean);
    const palabrasDespues = txtDespues.trim().split(/\s+/).filter(Boolean);
    const antes  = (palabrasAntes.length > PALABRAS ? "…" : "") +
                   palabrasAntes.slice(-PALABRAS).join(" ");
    const despues = palabrasDespues.slice(0, PALABRAS).join(" ") +
                    (palabrasDespues.length > PALABRAS ? "…" : "");

    const resultado = { antes, marcador, despues };
    contextosCamposCache[campo] = resultado;
    return resultado;
  } catch(e) {
    console.warn("[contexto]", e);
    contextosCamposCache[campo] = null;
    return null;
  }
}

/* Muestra u oculta el cuadro de contexto de un campo — estilo visor Word. */
async function mostrarContextoCampo(campo) {
  const ctxId = "ctx_" + campo.replace(/[^a-zA-Z0-9]/g, "_");
  const box   = document.getElementById(ctxId);
  if (!box) return;
  if (box.dataset.loaded === "1" && box.style.display !== "none") {
    box.style.display = "none"; return;
  }
  box.style.display = "block";
  if (box.dataset.loaded === "1") return;
  box.innerHTML = `<div class="campo-ctx-visor"><div class="word-page campo-ctx-page"><div class="loading-spinner" style="width:24px;height:24px;margin:16px auto;border-width:2px;"></div></div></div>`;
  const ctx = await extraerContextoCampo(campo);
  if (!ctx) {
    box.innerHTML = `<div class="campo-ctx-visor"><div class="word-page campo-ctx-page"><p style="color:var(--text-muted);font-size:9pt;margin:0;">No se encontró el marcador <strong>${esc(campo)}</strong> en el documento.</p></div></div>`;
    box.dataset.loaded = "1"; return;
  }
  const linea = `<p style="margin:0;word-break:break-word;">`
    + (ctx.antes  ? `<span style="color:#aaa;">${esc(ctx.antes)} </span>` : "")
    + `<mark style="background:#fff3cd;padding:0 3px;border-radius:3px;font-weight:700;color:var(--primary);">${esc(ctx.marcador)}</mark>`
    + (ctx.despues ? `<span style="color:#aaa;"> ${esc(ctx.despues)}</span>` : "")
    + `</p>`;
  box.innerHTML = `<div class="campo-ctx-visor"><div class="word-page campo-ctx-page">${linea}</div></div>`;
...
function buildCamposIAForm() {
  ...
  placeholdersIA.forEach(placeholder => {
    ...
    div.innerHTML = `
      <div class="ai-field-label">
        ...
        <span class="ai-badge">Redacción IA</span>
      </div>
      <textarea class="form-control campo-ia-input" data-placeholder="${esc(placeholder)}"
        rows="5" placeholder="Escribe aquí con tus propias palabras. Ejemplo:\nPRIMERO. Compré una casa el 5 de enero.\nSEGUNDO. La vendieron sin mi consentimiento.\n\nLa IA mejorará la ortografía y redacción formal.">${esc(camposIALlenados[placeholder]||"")}</textarea>
      <p class="ai-hint">Escribe de forma natural — usa Enter para separar cada hecho o pretensión (PRIMERO., SEGUNDO., etc.). La IA corregirá ortografía y redacción formal antes de incluirlo en el documento.</p>`;
    cont.appendChild(div);
  });
}
...
async function mejorarTextosConIA() {
  ...
  const systemPrompt =
    `Eres un abogado colombiano experto en redacción de demandas judiciales, derechos de petición, tutelas y contratos. ` +
    ...
    `CORRECTO: "PRIMERO. La demandante se encontraba en el lugar...\\nSEGUNDO. Al acercarse a la persona..." ` +
    `INCORRECTO: "PRIMERO.\\nLa demandante se encontraba..." — NUNCA pongas el número solo en una línea y el texto abajo. ` +
    ...
    `Usa vocabulario propio del derecho colombiano (v.gr., "el suscrito demandante", "en virtud de lo anterior", "consta en el expediente", "de conformidad con", etc.). ` +
    `Formato: "PRIMERO. [párrafo extenso del hecho]\\nSEGUNDO. [párrafo extenso del hecho]\\n..." ` +
    `Para PRETENSIONES: redacta como pretensiones numeradas formales, cada una clara, concisa y con fundamento jurídico implícito. Formato: "PRIMERO. [pretensión]\\nSEGUNDO. [pretensión]\\n..." ` +
...
async function regenerarCampoIA(clave) {
  ...
  const systemPrompt =
    `Eres un abogado colombiano experto en redacción de demandas judiciales. ` +
...
/* Redirige al login guardando el estado actual del formulario para retomarlo después. */
function pedirInicioSesion() {
  saveCamposActuales();
  pendingMinutaId       = currentMinuta ? currentMinuta.id : null;
  pendingCamposSnapshot = pendingMinutaId ? { ...camposLlenados } : null;
  cerrarModal();
...
document.getElementById("form-nueva-minuta").addEventListener("submit", async e => {
  ...
  try {
    ...
    const datosMinuta = {
      nombre, descripcion, categoria, precio, campos, camposLargo,
