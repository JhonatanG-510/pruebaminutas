...
function protegerLivePreviewAntiCopia() {
  ...
  window.addEventListener("blur", blackoutOn, true);
  window.addEventListener("focus", () => blackoutTemporal(600));

  // 3c) Detección PROACTIVA en keydown (capture phase) — apagamos antes de
  ...
  const isCaptureCombo = e => {
    ...
    // Cualquier pulsación con la tecla Windows (Meta) → herramientas de
    // captura del SO suelen activarse así (Win+Shift+S, Win+G, etc.)
    if (e.metaKey || code === "MetaLeft" || code === "MetaRight" ||
        code === "OSLeft" || code === "OSRight" ||
        (e.getModifierState && e.getModifierState("OS"))) return true;

    // Mac: Cmd+Shift+3/4/5/6
    if (e.shiftKey && (e.metaKey) && ["3","4","5","6"].includes(k)) return true;
  ...
  // capture=true → llegamos antes que cualquier otro handler de la página
  ...
  // 3d) En cuanto el ratón sale de la ventana (mover hacia barra de tareas
  //     a iniciar la herramienta de captura), apagamos preventivamente.
  document.addEventListener("mouseleave", blackoutOn);
  document.addEventListener("mouseenter", () => blackoutTemporal(400));
}
...
/* Engancha un listener delegado en el modal-body para refrescar la previsualización al escribir */
  ...
  body.addEventListener("input", e => {
    ...
    if (t.classList.contains("campo-input")) {
      ...
      if (k) {
        ...
        // Si el usuario edita después de que la IA mejoró este texto, invalidar el mejorado
